...
<title>Scooter</title>
@section('content')
    <div class="banner" style="width:100%; position: relative; height: 40vh; margin-top: -40px; background-image: url('https://github.com/Alex11520/img/blob/main/img/scooter_desktop.jpg?raw=true'); background-size: cover; background-position: center; border-radius: 1rem;">
        <div style="display: flex; height: 100%; align-items: center; justify-content: flex-end; padding-right: 10%; ">
            <p style="color: white; font-size: 20rem; font-weight: bold;">Scooter</p>
        </div>
    </div>
            ...
            @foreach ($scooters as $scooter)
                <div class="product-card">
                    <div class="product-image">
                        <img src="{{ $scooter->img }}" alt="product-image">
                    </div>
                    <div class="product-info">
                        <div class="product-name-price">
                            <span class="product-name">{{ $scooter->name }}</span>
                            <span class="product-price">A${{ number_format($scooter->price, 2) }}</span>
                        </div>
                        <div class="product-buttons">
                            <button data-id="{{ $scooter->prodNo }}" data-type="scooter" onclick="addToCart({{ $scooter->prodNo }})">Add to cart</button>
                            @if ($scooter->size)
                                <select name="size" id="size">
                                    <option value="{{ $scooter->size }}">{{ $scooter->size }}</option>
                                </select>
                            @endif
                            @if ($scooter->color)
                                <select name="colour" id="colour">
                                    <option value="{{ $scooter->color }}">{{ ucfirst($scooter->color) }}</option>
                                </select>
                            @endif
                        </div>
                    </div>
