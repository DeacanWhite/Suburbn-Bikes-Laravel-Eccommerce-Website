...
<title>Apparel</title>
@section('content')
    <div class="banner" style="width:100%; position: relative; height: 40vh; margin-top: -40px; background-image: url('https://github.com/Alex11520/img/blob/main/img/appareal_desktop.jpeg?raw=true'); background-size: cover; background-position: center; border-radius: 1rem;">
        <div style="display: flex; height: 100%; align-items: center; justify-content: flex-end; padding-right: 10%; ">
            <p style="color: white; font-size: 20rem; font-weight: bold;">Apparel</p>
        </div>
    </div>
            ...
            @foreach ($apparels as $apparel)
                <div class="product-card">
                    <div class="product-image">
                        <img src="{{ $apparel->img }}" alt="product-image">
                    </div>
                    <div class="product-info">
                        <div class="product-name-price">
                            <span class="product-name">{{ $apparel->name }}</span>
                            <span class="product-price">A${{ number_format($apparel->price, 2) }}</span>
                        </div>
                        <div class="product-buttons">
                            <button data-id="{{ $apparel->prodNo }}" data-type="apparel" onclick="addToCart({{ $apparel->prodNo }})">Add to cart</button>
                            @if ($apparel->size)
                                <select name="size" id="size">
                                    <option value="{{ $apparel->size }}">{{ $apparel->size }}</option>
                                </select>
                            @endif
                            @if ($apparel->color)
                                <select name="colour" id="colour">
                                    <option value="{{ $apparel->color }}">{{ ucfirst($apparel->color) }}</option>
                                </select>
                            @endif
                        </div>
                    </div>
